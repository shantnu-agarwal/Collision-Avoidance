...
public:
	...
	void chooseDirection() {
		...
		/*********COMPARING FEASIBILTY OF LEFT OVER RIGHT********/
...
void read_mpu_data() {                                             //Subroutine for reading the raw gyro and accelerometer data
	...
	Wire.requestFrom(0x68, 14);                                           //Request 14 bytes from the MPU
...
void setup() {
	...
	Wire.begin();
	setup_mpu_registers();
	for (int cal_int = 0; cal_int < 1000; cal_int++) {                  //Read the raw acc and gyro data from the MPU for 1000 times
		read_mpu_data();
